from Rcrontab.master.MasterPackages.sql_codes import get_start_flag, get_old_version, update_flag, \
    get_end_flag, get_execute_crawling_programs_result, update_version, get_execute_calculate_programs_result, \
    get_program_base_info, get_owner_info_from_sid
...
class LoopCheckProgramResult:
    def __init__(self):
        ...
        rerun_dict = r.get('crawling_program_rerun_dict')
        if rerun_dict:
    ...
    def crawling_procedure(self, execute_crawling_programs):
        for program in execute_crawling_programs:
            if int(program['event_type']) == 1:
                pass
            elif int(program['event_type']) == 2:
                self.success_procedure(program)
            elif int(program['event_type']) == 3:
                self.failed_procedure(program)

    def success_procedure(self, program):
        sid = program['sid']
        version = program['version']
        subversion = program['subversion']
        extra_info = program['extra_info']
        program_api = program['name']
        owner_name = program[ 'owner']
        mail = program['mail']
        send_mail = False
        if extra_info:
    ...
    def failed_procedure(self, program):
    ...
    def _get_max_flag():
        ...
        try:
            ...
            try:
                ...
                flag = (start_id, end_id)
                return flag

    @staticmethod
    def _get_execute_crawling_programs(start_id, end_id):
        execute_crawling_programs = list()
        if start_id == end_id:
            pass
        else:
            sql = mysql.Mysql()
            df = sql.mysql_read(get_execute_crawling_programs_result.format(start_id=start_id, end_id=end_id))
            if len(df) == 0:
                pass
            else:
                for idx in df.index:
                    info = df.loc[idx, ['id', 'sid', 'version', 'subversion', 'extra_info', 'event_type', 'program_type'
                                        'name', 'owner','mail']].to_dict()
                    execute_crawling_programs.append(info)
        return execute_crawling_programs
    ...
    @staticmethod
    def _get_execute_calculate_programs(start_id, end_id):
        execute_calculate_programs = list()
        if start_id == end_id:
            pass
        else:
            sql = mysql.Mysql()
            df = sql.mysql_read(get_execute_calculate_programs_result.format(start_id=start_id, end_id=end_id))
            if len(df) == 0:
                pass
            else:
                for idx in df.index:
                    info = df.loc[idx, ['id', 'sid', 'version', 'subversion', 'extra_info', 'event_type', 'program_type'
                                        'name', 'owner','mail']].to_dict()
                    execute_calculate_programs.append(info)
        return execute_calculate_programs
